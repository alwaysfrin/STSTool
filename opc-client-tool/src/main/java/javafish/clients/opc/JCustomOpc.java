...
import com.opc.demo.OpcdemoApplication;
...
abstract public class JCustomOpc {
  ...
  static {
    ...
    //classes中查询dll（java -jar运行时可能无法获取路径）
    String filePath = Thread.currentThread().getContextClassLoader().getResource("JCustomOpc.dll").getPath();
    ...
    //需要将dll放在classpath中（jre/bin中）
    //System.loadLibrary("JCustomOpc");
    
    //加载绝对路径，如：C:\\JCustomOpc.dll
    //System.load(props.getProperty("library.path"));
  }
